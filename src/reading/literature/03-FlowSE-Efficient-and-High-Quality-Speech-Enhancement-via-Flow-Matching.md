...
- 从高斯初始化 $M_0$ 出发
- 多次调用速度网络 $v_\theta(\cdot)$
- 用 ODE solver 反复更新状态 $M_t$
- 最终得到增强 mel $\hat M_x$
...
1. **要学的东西是什么**：一个速度场（velocity field）

$$
\frac{dz_t}{dt} = v_\theta(z_t, t),\quad z_0=y,\ z_1=x
$$

也就是学 $v_\theta$ 这个函数。

2. **推理/生成时怎么用**：用 ODE 求解（多步更新）

$$
z_{t+\Delta t}=z_t+v_\theta(z_t,t)\Delta t
$$

3. **训练时怎么学**：用 flow-matching loss 去逼近“正确的速度”（论文后面把它叫 probability flow / velocity field）。
...
h = 10
$$
当开启 dropout 时（丢弃概率 $p=0.1$）

训练时每次 forward 会随机一个 mask $m$：
...
$$
\mathbb E[\tilde h] = 0.9\cdot 10 + 0.1\cdot 0 = 9
$$
原本应该直接输出 10 的，但是有了 dropout，输出的平均可能就变成了 9 。这就叫**期望偏移**：
**原来重复多次的话“平均”是 10，现在因为随机丢弃，平均变成了 9（变小了 $(1-p)$ 倍）。**
$$
\mathbb E[\tilde h] = (1-p)\,h
$$
...
把 dropout 后输出改成（参考上一节的公式 $\mathbb E[\tilde h] = (1-p)\,h$ ）：
$$
\tilde h = \frac{m}{1-p}\,h
$$
也就是说：
...
$$
\mathbb E[\tilde h]
=0.9\cdot \frac{10}{0.9} + 0.1\cdot 0
=10
$$
